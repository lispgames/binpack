    ...
    (let* ((x1 0)
      ...
      (glim:with-pushed-matrix (:modelview)
        ...
        (when (and (placing w) (hole w))
          (flet ((s (x) (* (spacing w) (floor x (spacing w)))))
            (let* ((mx (s (gmx w)))
                   (my (s (gmy w)))
                   (x1 (1+ x1))
                   (y1 (1+ y1))
              ...
              (when (and (plusp mx) (plusp my))
                (glim:with-draw (:quads :shader :solid)
                  (color 0 1 0)
                  (let ((px (binpack::find-all-placements (hole w) mx my)))
                    (loop for p in px
                          for x = (binpack::x p)
                          for y = (binpack::y p)
                          do (vertex (+ x in) (+ y in))
        ...
        (dispatch-draws w)
        (when (shapes w)
          (loop for s in (shapes w) do (draw-shape w s)))
        (dispatch-draws w)))
    (when (show-colors w)
      (glim:with-pushed-matrix (:modelview)
        (glim:load-identity)
...
(defmethod redraw-hole ((w binpack2-vis))
  (when (hole w)
    (let ((*w* w))
      (draw-hole (hole w) (aref *flags* 7))
      (when (aref *flags* 8)
        (draw-cd (hole w) (pwx w)))
      (when (aref *flags* 9)
        (draw-placements (hole w) (pwx w) (pwy w))))))
    ...
    (case button
      (:left-button
       (cond
         ...
         ((placing w)
          (setf (pwx w) (max 1 (floor (gmx w) (spacing w))))
          (setf (pwy w) (max 1 (floor (gmy w) (spacing w))))
